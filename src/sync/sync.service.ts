...
export class SyncService {
  ...
  async pushChanges(userId: string, pushDto: PushChangesDto): Promise<{
    processed: number;
    conflicts: any[];
    cursor: string;
  }> {
    ...
    for (const change of pushDto.changes) {
      if (change.entityType === 'task') {
        const serverTask = change.operation === 'update'
        ...
        const result = await this.outboxProcessor.processOutbox(userId, [change]);
  ...
  async pullChanges(userId: string, pullDto: PullChangesDto): Promise<{
    changes: any[];
    cursor: string;
  }> {
