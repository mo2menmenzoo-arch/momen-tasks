  ...
  async process(job: Job): Promise<any> {
    switch (job.name) {
      case 'generate-weekly-review':
        return this.handleGenerateWeeklyReview(job as Job<{ userId: string; weekStart: string }>);
      default:
        throw new Error(`Unknown job name: ${job.name}`);
    }
  }

  private async handleGenerateWeeklyReview(job: Job<{ userId: string; weekStart: string }>) {
    this.logger.log(
      `Generating weekly review for user ${job.data.userId} for week starting ${job.data.weekStart}`,
    );

    try {
      const review = await this.clarityEngineService.getWeeklyReview(job.data.userId);
      ...
      this.logger.log(
        `Weekly review generated for user ${job.data.userId}`,
      );
    } catch (error: any) {
      this.logger.error(
